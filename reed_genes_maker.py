...
def ann_maker(orf, orfdict):
	...
	seq = subprocess.check_output(['tail', '-1', 'real_contigs/%s.fa'%orf])
	###for full genome	
	...
	print '#insertFile ehv2/real_contigs/%s_bplink.html'%gene
...
if __name__ == "__main__":
	...
	for i in result.keys():
		gene= i
		htmlfile=open('real_contigs/%s_bplink.html'%gene,'w')
		htmlfile.write('<br><h3><a href="http://etna.mssm.edu/chris/web/dna_mth/pile_up_view.pl?fa=out/ehv2/real_contigs/1.%s.fa&bp=out/ehv2/real_contigs/1.%s.mvi.bp&mid=101&flank=100">Pileup</a></h3>'%(gene,gene)) 
		htmlfile.close()
        	saveout=sys.stdout
        	outfile = open('real_contigs/%s.ann'%gene,'w')
        	sys.stdout=outfile
